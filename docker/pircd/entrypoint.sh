...
for r in releases:
    ...
    if has_pircd:
        # Strip leading "pircd-v" or "v" prefix to get a bare semver
        ver = tag
...
# ── download and install pircd binary ────────────────────────────────────────

install_pircd() {
    local version="$1"
    local target="$2"
    local archive="pircd-v${version}-${target}.tar.gz"
    local url="https://github.com/${REPO}/releases/download/v${version}/${archive}"
    local tmp
    tmp=$(mktemp -d)

    log "Downloading pircd v${version} for ${target}..."
    ...
    tar -xzf "${tmp}/${archive}" -C "${tmp}" pircd
    mv "${tmp}/pircd" "${BINARY}"
    chmod +x "${BINARY}"
    rm -rf "${tmp}"
    echo "${version}" > "${VERSION_FILE}"
    log "Installed pircd v${version}"
}
...
main() {
    ...
    # Install binary if missing or no version recorded
    local current_version=""
    [ -f "${VERSION_FILE}" ] && current_version=$(cat "${VERSION_FILE}")

    if [ ! -x "${BINARY}" ] || [ -z "${current_version}" ]; then
        log "Fetching latest pircd release..."
        ...
        install_pircd "${latest_version}" "${target}"
    else
        log "Using pircd v${current_version}"
    fi
